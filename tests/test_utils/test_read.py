...
def test_fct__parse_line_blind(row, floats_only, expected_response):
    ...
    for index in range(0, len(expected_response)):
        assert isinstance(parse_line_blind(row=row, floats_only=floats_only)[index], type(expected_response[index]))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ABC\t123\t120.2", "ABC\t123\t120.2", "DEF\t123\t120.2"], "\t", True, 1, [("ABC", 123.0, 120.2)], (str, float, float)),
        (["ABC 123 120.2", "ABC 123 120.2"], " ", True, 1, [("ABC", 123.0, 120.2)], (str, float, float)),
        (["ABC 123 120.2", "ABC 123 120.2"], " ", False, 1, [("ABC", 123.0, 120.2)], (str, int, float)),
    ],
)
def test_fct__stream_tuples(csv_input, separator, floats_only, maxrecords, expected_output, expected_types):
    test_fct__stream_tuples.__test_type__ = "fct"
    stream = stream_tuples(input=csv_input, fs=separator, floats_only=floats_only, preload=True, maxrecords=maxrecords)
