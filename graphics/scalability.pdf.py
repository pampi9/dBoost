...
from utils import filename, rcparams, save2pdf, setup, to_inches
...
setup()
rcparams()
pyplot.gcf().set_size_inches(
    to_inches(240), to_inches(240)
)  # full column size is 240pt
...
for e, (tr, _tr) in itertools.product(es, zip(trs, _trs)):
    ...
    ax.plot(
        ...
        results[(e[1], tr)],
        next(linecycler),
        label="{}, {}".format(e[1].capitalize(), _tr),
