...
def get_marker_params(color, **kwargs):
    return dict(
        facecolor="none", edgecolor=TANGO[color][2], alpha=MARKER_ALPHA, **kwargs
    )
    # must use facecolor: see https://github.com/matplotlib/matplotlib/issues/4083


INLIER3D = dict(
    linewidth=EDGE_WIDTH, s=BASE_SIZE, marker="o", **get_marker_params("green")
)
OUTLIER3D = dict(
    linewidth=EDGE_WIDTH, s=BASE_SIZE * 2, marker="x", **get_marker_params("red")
)
...
sensors_schema = [
    r"Temperature (\si{\degreeCelsius})",
    r"Humidity (\si{\percent})",
    r"Light (\si{\lux})",
    r"Voltage (\si{\volt})",
]
...
def sensors3D(dfile, ofile):
    ...
    for subplot_id, columns in enumerate(sorted(columnsets)):
        sensors3D_one(
            fig,
            (nb_plots // COLUMNS, COLUMNS, 1 + subplot_id),
            inliers,
            outliers,
            *columns
        )
