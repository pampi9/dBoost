...
from utils import TANGO, filename, rcparams, to_inches
...
matplotlib.rcParams["text.latex.preamble"] = [
    r"\usepackage{amsmath,amssymb,calc}\usepackage{pifont}\newcommand{\cmark}{\ding{51}}\newcommand{\xmark}{\ding{55}}"
]
...
fig, axes = pyplot.subplots(
    len(HISTOGRAMS) // PLOTS_PER_LINE,
    PLOTS_PER_LINE,
    sharex=True,
    sharey=True,
    squeeze=False,
    frameon=False,
    figsize=(to_inches(504), 1.75),
)
...
def makehatches(axis, bar, in_di_peak, in_dd_peak):
    ...
    for hid in range(-3, int(h / h_dist) + 3):
        (xx0, yy0), (xx1, yy1) = (x0, y0 + hid * h_dist - h_dist), (
            x1,
            y0 + hid * h_dist + h_dist,
        )
        drawline(
            axis,
            (xx0, xx1),
            (yy0, yy1),
            bar,
            colors[hid % len(colors)],
            dashes[hid % len(dashes)],
        )
...
def arrow(axis, x1, y1, x2, y2, arrow_style, color="black", line_style="solid"):
    ...
    axis.annotate(
        ...
        arrowprops={"arrowstyle": arrow_style, "linestyle": line_style, "color": color},
    )


def markers(axis, x1, x2, y, color, dashes, msize=0.02):
    lines = (
        Line2D((x1, x2), (y, y), color=color, dashes=DASHES[dashes], clip_on=False),
...
for hid, hist in enumerate(HISTOGRAMS):
    ...
    annot_color = (
        "black"  # GREEN if delta >= 5 else RED '$\boldsymbol\times\mathbf{{{:.0f}}}$'
    )
    axis.text(
        annot_x - 0.5,
        annot_y,
        r"$\times{:.0f}$".format(delta),
    ...
    label = FLABEL.format(
        header(False), peaked(Histogram), header(True), peaked(PartitionedHistogram)
    )
...
legend = fig.legend(
    ...
    labels=["top bins ($D$-independent model)", "top bins ($D$-dependent model)"],
    loc=8,
    ncol=2,
