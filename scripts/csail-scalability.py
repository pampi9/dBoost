...
def get_args(training_path, dataset_path, printout_delay):
    cmd = "../dboost/dboost-stdin.py --statistical 1 --histogram 0.8 0.2 --in-memory --pr {} --train-with".format(
        printout_delay
    )
...
def time(training_sizes, dataset_size, printout_delay):  # --pr
    ...
    for training_size in training_sizes:
        ...
        results_path = "../results/csail/csail-timings-{}-{}.txt".format(
        ...
        with open(results_path, mode="w") as resfile:
            ...
            print(
                "Running: dataset size {}, training size {}...".format(
                    dataset_size, training_size
                )
            )
            subprocess.Popen(
                args, bufsize=-1, stderr=resfile, stdout=subprocess.DEVNULL
            ).communicate()
