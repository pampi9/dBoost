...
class Cords:
    ...
    def fit(self, Xs):
        Xs = list(
            Xs
        )  # We're going to do two passes, so make sure we don't consume all the data during the first one.
        ...
        for nb, X_ in enumerate(Xs):
            ...
            for (X, Y), (nx, ny) in zip(
                itertools.combinations(X_, 2), itertools.combinations(range(len(X_)), 2)
            ):
                for (x, y), (nnx, nny) in zip(
                    itertools.product(zip(*[X]), zip(*[Y])),
                    ...
                    if d1 == 1 or d2 == 1 or d1 == float("+inf") or d2 == float("+inf"):
                        continue
                    N, Nx, Ny = (
                        addlist2d(N, num, d1, d2),
                        addlist(Nx, num, d1),
                        addlist(Ny, num, d2),
                    )
        ...
        for (X, Y), (nx, ny) in zip(
            ...
            for (x, y), (nnx, nny) in zip(
                ...
                v = (d1 - 1) * (d2 - 1)
                n = (
                    math.sqrt(-16 * v * math.log(self.p * math.sqrt(2 * math.pi)))
                    - 8 * math.log(self.p * math.sqrt(2 * math.pi))
                ) / (1.69 * self.delta * (d - 1) * pow(v, -0.071))
                ...
                t = 1 / (
                    ...
                    * (
                        1
                        + math.erf(
                            ((1 - self.p) - (v + lda)) / (math.sqrt(2 * v + 4 * lda))
                        )
                    )
                )
