...
def descriptions(ruleset):
    ...
    for type in ruleset:
        ...
        for rule in ruleset[type]:
            descriptions[type].extend(
                inspect.getfullargspec(rule).annotations["return"]
            )
...
HTML5_EMAIL_VALIDATOR = re.compile(
    r"^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.(?P<ext>[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?))*$"
)
...
def email_checks(s: str) -> ("simple email check",):  # "RFC822 email check"):
    return (
        HTML5_EMAIL_VALIDATOR.match(s) != None,
