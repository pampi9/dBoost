...
class Mixture:
    ...
    def find_discrepancies(self, X, index):
        ...
        for id, (correlation, gmm_pos, cutoff) in enumerate(
            zip(correlations, range(len(self.gmms)), range(len(self.gmms)))
        ):
    ...
    def more_info(self, discrepancy, description, X, indent="", pipe=sys.stdout):
        ...
        gmm = self.gmms[gmm_pos]
        mean, covar, weight = (
            gmm.means_[component],
            gmm.covars_[component],
        ...
        pipe.write(
            indent
            + "• Probability of being generated by best match Gaussian: {:.2e} (threshold: {})\n".format(
                prob, self.cutoff
            )
        )
